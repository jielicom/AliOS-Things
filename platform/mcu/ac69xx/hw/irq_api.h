...
#define GET_IRQ_ENTRY_NAME(hdl) __pi32_interrupt_ ## irq_ ## hdl ## _entry

#define IRQ_REGISTER(idx, hdl) \
    extern void GET_IRQ_ENTRY_NAME(hdl)(void); \
...
#define IRQ_REQUEST(idx, hdl) \
    irq_handler_register(idx, GET_IRQ_ENTRY_NAME(hdl), irq_index_to_prio(idx))
